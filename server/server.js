...
function freeze(target, properties) {
    properties.forEach(property =>
        Object.defineProperty(target, property, {configurable: false, writable: false})
    );
}
...
function merge(target, source) {
    freeze(target, ["constructor", "prototype", "__proto__"]);
    ...
    socket.on(events.COMMAND, function (data) {
        ...
        switch (data.command) {
            ...
            case commands.CHANGE_DATA:
                const merged = merge(socket, data.updated);
                if (merged) {
                    socket = merged;
                    response = true;
                } else {
